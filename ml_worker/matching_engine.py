...
def persist_proposals(report_id,results):
    with connect() as conn:
        ...
        for index,(event,candidates,routing,reasons,missing,version) in enumerate(results):
            ...
            conn.execute('INSERT INTO staged_proposals(report_event_id,project_id,schedule_version_id,selected_activity_id,candidate_matches,proposed_changes,expected_activity_row_version,match_score,model_version,routing_status,missing_fields,reason_codes,explanation,raw_scores) VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)',(row['id'],report['project_id'],version,top['id'] if top else None,Jsonb(safe),Jsonb(event.model_dump(mode='json')),top['row_version'] if top else None,top['rerank_score'] if top else None,CROSS_MODEL+'@'+CROSS_REVISION,routing,Jsonb(missing),Jsonb(reasons),'Local semantic retrieval and raw cross-encoder ranking. Reviewer approval required.',Jsonb({'retrieval':top['retrieval_score'],'rerank':top['rerank_score']} if top else {})))
...
def process_report(report_id):
    ...
    for event in events:
        ...
        candidates=rerank_candidates(event,apply_hard_rules(event,candidates))
