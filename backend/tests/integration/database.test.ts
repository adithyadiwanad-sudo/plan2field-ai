...
test('schema and role protection on a disposable real PostgreSQL database',{skip:!url},async()=>{
 ...
 try{
  ...
  await c.query(await readFile(new URL('../../../database/migrations/003_field_evidence.sql',import.meta.url),'utf8'));
  await c.query(await readFile(new URL('../../../database/migrations/004_demo_profiles.sql',import.meta.url),'utf8'));
