...
export async function closeout(
  ...
  return transaction(async (c) => {
    ...
    await c.query(
      "INSERT INTO historical_activity_records(project_id,activity_id,work_type,discipline,quantity,unit,context,baseline_duration,actual_duration,duration_basis,date_evidence,deviation_reason,provenance,approved_by,approved_at,closeout_version) SELECT a.project_id,a.id,a.activity_type,COALESCE(a.discipline,'UNKNOWN'),a.accepted_quantity,a.quantity_unit,jsonb_build_object('area',a.area,'progress_pattern',COALESCE((SELECT jsonb_agg(jsonb_build_object('event_type',e.event_type,'effective_date',e.effective_date,'physical_percent',e.after_state->>'physical_percent_complete','source_event_id',e.id) ORDER BY e.effective_date,e.committed_at) FROM progress_events e WHERE e.activity_id=a.id),'[]'::jsonb)),b.baseline_finish-b.baseline_start,a.actual_finish-a.actual_start,'CALENDAR_DAYS',jsonb_build_object('baseline_start',b.baseline_start,'baseline_finish',b.baseline_finish,'actual_start',a.actual_start,'actual_finish',a.actual_finish),$3,$4,$5,now(),1 FROM schedule_activities a JOIN activity_baselines b ON b.activity_id=a.id WHERE a.project_id=$1 AND a.schedule_version_id=$2 AND b.baseline_start IS NOT NULL AND b.baseline_finish IS NOT NULL ON CONFLICT DO NOTHING",
      [
        project,
